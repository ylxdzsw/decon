...
use syn::{Expr, ExprCall, ExprClosure, ExprPath, Path, Stmt, __private::ToTokens, parse_quote, punctuated::Punctuated, token::{Move, Or, Paren}, visit_mut::{self, VisitMut}};
use uuid::Uuid;
...
fn _reset(input: TokenStream) -> TokenStream {
    ...
    let mut stmts = mem::take(&mut func.block.stmts);
...
// TODO: split into crates and export Cont type definitions (and ContOwnd, ContMut etc.) (because Rust don't allow export anything else in a proc_macro crate)
